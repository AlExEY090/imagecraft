...
struct Pixel formula_transform(int x, int y, struct BMPImage *img, void * params) {
    ...
    struct Pixel p=img->data[(int64_t)y*img->infoHeader.biWidth+x];
...
struct Pixel matrix_transform(int x, int y, struct BMPImage *img, void * params) {
    ...
    return (struct Pixel){
        (uint8_t)fmaxf(0, fminf(255, newB)),
        (uint8_t)fmaxf(0, fminf(255, newG)),
        (uint8_t)fmaxf(0, fminf(255, newR))
    };
}
...
struct CrystalParams* create_crystal_params(int count, int width, int height) {
    ...
    for (int i = 0; i < count; i++) {
        p->coords_x[i] = rand() % width;
        p->coords_y[i] = rand() % height;
    }
    return p;
}
struct Pixel transformer_crystallize(int x, int y, struct BMPImage *img, void *params) {
    ...
    return img->data[cy * img->infoHeader.biWidth + cx];
}
struct BMPImage* crop_image(struct BMPImage *src, void *params) {
    ...
    new_img->fileHeader = src->fileHeader;
    new_img->infoHeader = src->infoHeader;
    ...
    int y_offset = src_h - p->new_height;

    for (int y = 0; y < p->new_height; y++) {
        for (int x = 0; x < p->new_width; x++) {
            new_img->data[y * p->new_width + x] = src->data[(y + y_offset) * src_w + x];
        }
    }
